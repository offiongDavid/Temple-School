...
const API_URL = "https://sheetdb.io/api/v1/flf4gxij1zvsp";
...
function renderEvents() {
  ...
  if (upcoming.length === 0) {
    upcomingGrid.innerHTML = "<p class='text-gray-500'>No upcoming events</p>";
  }

  if (past.length === 0) {
    pastGrid.innerHTML = "<p class='text-gray-500'>No past events</p>";
  }
...
function createEventCard(e, isUpcoming) {

  const card = document.createElement("div");

  card.className = "bg-white rounded-xl overflow-hidden shadow group cursor-pointer";
  ...
  card.innerHTML = `
    <div class="relative overflow-hidden">
      ...
      ${
        isUpcoming
        ? `<span class="absolute top-3 left-3 bg-teal-500 text-white text-xs px-3 py-1 rounded-full">
            Upcoming
          </span>`
        : `<span class="absolute top-3 left-3 bg-gray-400 text-white text-xs px-3 py-1 rounded-full">
            Past
          </span>`
      }
    </div>
    ...
    <div class="p-5">
      <h3 class="text-[#0B1C3D] font-semibold text-lg">${e.title}</h3>
      <p class="text-[#C8A96A] text-sm mt-1">${e.date}</p>
      <p class="text-gray-600 text-sm mt-3">${e.description}</p>
    </div>
  `;
